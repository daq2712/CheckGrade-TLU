    ...
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            ...
            if grades_response.status_code == 200:
                print("Lấy bảng điểm thành công")
                break
            else:
                print(f"Không thể lấy bảng điểm ({grades_response.status_code}). Thử lại lần {attempt}/{MAX_RETRIES} sau {RETRY_DELAY} giây...")
                time.sleep(RETRY_DELAY)

        except httpx.RequestError as e:
            print(f"Lỗi kết nối khi lấy bảng điểm: {e}. Thử lại lần {attempt}/{MAX_RETRIES} sau {RETRY_DELAY} giây...")
            time.sleep(RETRY_DELAY)

    if not grades_response or grades_response.status_code != 200:
        print("Không thể lấy bảng điểm sau nhiều lần thử.")
        exit()
